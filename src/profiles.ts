...
import { AppleApiClient } from './apple-api';
import { TargetSigningInfo } from './xcode-project';

function normalizeSerial(serial: string): string {
  return serial.replace(/^0+/, '').toUpperCase();
}

function profileNeedsRenewal(expirationDateRaw: unknown, minValidityDays: number): boolean {
...
async function parseProvisioningProfileUuid(profilePath: string): Promise<string | null> {
  let xml = '';
...
export async function generateAndInstallProfiles(params: {
  appleClient: AppleApiClient;
  targets: TargetSigningInfo[];
  distributionMethod: 'app-store' | 'ad-hoc' | 'development';
  profileType: string;
  teamId: string;
  certificateSerialNumber: string;
  minProfileValidityDays: number;
}): Promise<void> {
  ...
  if (!certificate) {
    throw new Error(
      `Could not find distribution certificate in App Store Connect with serial ${params.certificateSerialNumber}.`,
    );
  }

  const devices =
    params.distributionMethod === 'app-store' ? [] : await params.appleClient.listDevices();
